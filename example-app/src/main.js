...
},
{
              ...
              inputs: [{ name: 'productIdentifiers', label: 'Product identifiers', type: 'text', value: 'premium_upgrade' }],
              run: async (values) => {
                const raw = values.productIdentifiers || '';
const productIdentifiers = raw.split(',').map((id) => id.trim()).filter(Boolean);
